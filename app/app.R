...
# Data quality metrics generated by the rolls-metadata pipeline.
# Script 08 creates two files:
#   1. dashboard_data_quality.csv         = study-level summary
#   2. dashboard_dataset_data_quality.csv = dataset-level detail

data_quality_path <- "data/dashboard_data_quality.csv"
dataset_data_quality_path <- "data/dashboard_dataset_data_quality.csv"

if (!file.exists(data_quality_path)) {
  stop(
    paste0(
      "The study-level dashboard data quality CSV was not found.\n\n",
      "Run the rolls-metadata pipeline through Script 08, then copy:\n",
      "outputs/dashboard_data_quality.csv\n\n",
      "to:\n",
      "app/data/dashboard_data_quality.csv"
    )
  )
}

if (!file.exists(dataset_data_quality_path)) {
  stop(
    paste0(
      "The dataset-level dashboard data quality CSV was not found.\n\n",
      "Run the rolls-metadata pipeline through Script 08, then copy:\n",
      "outputs/dashboard_dataset_data_quality.csv\n\n",
      "to:\n",
      "app/data/dashboard_dataset_data_quality.csv"
    )
  )
}
...
dataset_data_quality <- read_csv(
  dataset_data_quality_path,
  show_col_types = FALSE,
  na = c("", "NA")
)
...
data_quality_missing <- setdiff(
  data_quality_required,
  names(data_quality)
)

dataset_data_quality_missing <- setdiff(
  dataset_data_quality_required,
  names(dataset_data_quality)
)

if (length(data_quality_missing) > 0) {
  stop(
    paste(
      "The study-level dashboard data quality CSV is missing these required columns:",
      paste(data_quality_missing, collapse = ", ")
    )
  )
}

if (length(dataset_data_quality_missing) > 0) {
  stop(
    paste(
      "The dataset-level dashboard data quality CSV is missing these required columns:",
      paste(dataset_data_quality_missing, collapse = ", ")
    )
  )
}
...
data_quality <- data_quality %>%
  mutate(
    ...
    study_display = str_to_title(
      str_replace_all(study, "_", " ")
    )
  )
...
data_quality_study_choices <- sort(
  unique(data_quality$study_display)
)
    ...
    fluidPage(
      ...
      wellPanel(
        selectInput(
          ...
          selected = if (length(data_quality_study_choices) > 0) {
            data_quality_study_choices[1]
          } else {
            character(0)
          }
        )
...
server <- function(input, output, session) {
  ...
  quality_selected <- reactive({
    req(input$quality_study)
    
    data_quality %>%
      filter(
        study_display == input$quality_study
      )
  })
  
  dataset_quality_selected <- reactive({
    req(input$quality_study)
  ...
  output$data_quality <- renderDT({
    quality_table <- quality_selected() %>%
      transmute(
        `Data group`,
  ...
  output$macro_status <- renderUI({
    ...
    tags$div(
      ...
      tags$strong("Intake macros computed: "),
      status
    )
  })
}
