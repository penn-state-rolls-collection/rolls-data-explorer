...
# Study-level data quality metrics generated by the rolls-metadata pipeline.
data_quality_path <- "data/dashboard_data_quality.csv"

if (!file.exists(data_quality_path)) {
  stop(
    paste0(
      "The dashboard data quality CSV was not found.\n\n",
...
if (length(data_quality_missing) > 0) {
  stop(
    paste(
      "The dashboard data quality CSV is missing these required columns:",
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    fluidPage(
      ...
      p(
        "Select a study to view missingness and completeness metrics for all data, ",
        "intake data, questionnaire data, and total intake variables."
      ),
      wellPanel(
        selectInput(
  ...
  output$data_quality <- renderDT({
    quality_table <- quality_selected() %>%
      transmute(
        ...
        `# participants with at least 85% complete data` = if_else(
          is.na(`# participants with at least 85% complete data`),
          "Not applicable",
          as.character(`# participants with at least 85% complete data`)
        )
      )
